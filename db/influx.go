...
type InfluxStorageService struct {
	...
	dataChannel  chan libs.InfluxIngestable
}
...
func NewInfluxStorage(params map[string]string) (DataWriter, error) {
	...
	res := &InfluxStorageService{
		cli:          cli,
		dataChannel:  make(chan libs.InfluxIngestable, 10000),
...
func (c *InfluxStorageService) Write(data interface{}) {
	if ii, ok := data.(libs.InfluxIngestable); ok {
		c.dataChannel <- ii
	} else {
...
func (i *InfluxStorageService) Ingest() {
	var data []libs.InfluxMeasurement
	...
	for {
		...
		case <-ticker:
			go i.process(data)
			data = []libs.InfluxMeasurement{}
		}
	}
}

func (i *InfluxStorageService) process(data []libs.InfluxMeasurement) {
